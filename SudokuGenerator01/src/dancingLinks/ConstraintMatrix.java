...
public class ConstraintMatrix {
    ...
    */
    ...
    private void fillMatrixRow(int matrixRowIndex, int rowNumber, int colNumber, int value) {
        rowConstraint = matrix[matrixRowIndex][calcRowIndex(rowNumber, value)] = new Node(rowNumber, colNumber, value);
        colConstraint = matrix[matrixRowIndex][calcColIndex(colNumber, value)] = new Node(rowNumber, colNumber, value);
        boxConstraint = matrix[matrixRowIndex][calcBoxIndex(rowNumber, colNumber, value)] = new Node(rowNumber, colNumber, value);
        cellConstraint = matrix[matrixRowIndex][calcCellIndex(rowNumber, colNumber)] = new Node(rowNumber, colNumber, value);
    }

    private int calcRowIndex(int row, int val) {
        return row * 9 + val;
    }

    private int calcColIndex(int col, int val) {
        // colOffset
        return 81 + (col * 9 + val);
    }

    private int calcBoxIndex(int row, int col, int val) {
        // boxOffset
